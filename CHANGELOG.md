...
- Большие Scriptable‑виджеты «Сегодня» и «Завтра» отдают основную высоту заездам: до пяти крупных строк сохраняют имя гостя, квартиру, источник и число ночей, а компактные выезды закреплены у нижнего края.
...
- Технический Booking.com `CLOSED`-маркер, начинающийся ровно в дату выезда подтверждённого Booking‑гостя, больше не попадает в новые заезды без имени.
- Сокращающийся Booking.com `CLOSED`-маркер больше не наследует начало объединённого периода предыдущего гостя: новый заезд остаётся видимым как операционная Booking‑бронь даже без имени в iCal, а устаревшие технические снимки с тем же выездом сразу мягко архивируются.
