...
function isBlocked(booking) {
  return isUnavailableMarker(booking) && !isOperationalBookingFallback(booking);
}
...
function formatEntry(booking) {
  ...
  return {
    ...
    operational_fallback: isOperationalBookingFallback(booking),
  };
}
...
async function buildTodayWidgetPayload(db, targetDate = todayRome()) {
  ...
  for (const booking of bookings) {
    ...
    if (start < targetDate && targetDate < end && (entry.guest !== '—' || entry.operational_fallback)) occupied.push(entry);
  }
