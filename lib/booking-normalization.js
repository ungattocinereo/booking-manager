...
function bookingDatesDiffer(a, b) {
  return dateOnly(a?.start_date) !== dateOnly(b?.start_date) ||
    dateOnly(a?.end_date) !== dateOnly(b?.end_date);
}

function bookingDateRangesAreSimilar(a, b) {
  const aStart = dateMs(a?.start_date);
  const aEnd = dateMs(a?.end_date);
  const bStart = dateMs(b?.start_date);
  const bEnd = dateMs(b?.end_date);
...
function markerOnlyTrimsElapsedStart(marker, reservation) {
  const markerStart = dateMs(marker?.start_date);
  const reservationStart = dateMs(reservation?.start_date);
  return dateOnly(marker?.end_date) === dateOnly(reservation?.end_date) &&
    markerStart != null &&
    reservationStart != null &&
    markerStart > reservationStart;
}

function buildCalendarReconciliations(bookings = []) {
  const activeBookings = bookings.filter(isActiveBooking);
  const realBookings = activeBookings
    .filter(booking => isRealReservationCandidate(booking, activeBookings));
  const markers = activeBookings.filter(booking => {
    if (String(booking?.platform || '').toLowerCase() !== 'booking') return false;
    if (!isUnavailableMarker(booking) || isGroupedBookingClosure(booking, activeBookings)) return false;
    const nights = nightsBetween(booking.start_date, booking.end_date);
    return Boolean(nights && nights <= BOOKING_FALLBACK_MAX_NIGHTS);
  });

  const matchesByMarker = new Map(markers.map(marker => [
    marker,
    realBookings.filter(real =>
      real.property_id === marker.property_id &&
      dateRangesOverlap(marker.start_date, marker.end_date, real.start_date, real.end_date)
    )
  ]));
  const matchesByReal = new Map(realBookings.map(real => [
    real,
    markers.filter(marker =>
      marker.property_id === real.property_id &&
      dateRangesOverlap(marker.start_date, marker.end_date, real.start_date, real.end_date)
    )
  ]));

  const byMarker = new Map();
  const supersededReservations = new Set();
  for (const marker of markers) {
    const matches = matchesByMarker.get(marker) || [];
    if (matches.length !== 1) continue;
    const reservation = matches[0];
    if ((matchesByReal.get(reservation) || []).length !== 1) continue;
    if (!bookingDatesDiffer(marker, reservation)) continue;
    if (!bookingDateRangesAreSimilar(marker, reservation)) continue;
    if (markerOnlyTrimsElapsedStart(marker, reservation)) continue;
    byMarker.set(marker, reservation);
    supersededReservations.add(reservation);
  }

  return { byMarker, supersededReservations };
}

function mergeCalendarDatesWithGuest(marker, reservation) {
  return {
    ...marker,
    raw_summary: reservation.raw_summary || marker.raw_summary,
    guest_name: reservation.guest_name || null,
    guest_country: reservation.guest_country || null,
    guest_count: Number(reservation.guest_count) || null,
    reservation_url: reservation.reservation_url || marker.reservation_url || null,
    phone_last4: reservation.phone_last4 || marker.phone_last4 || null,
    created_at: reservation.created_at || marker.created_at,
...
function followsConfirmedBookingCheckout(booking, allBookings = []) {
  if (!booking?.property_id) return false;
  return allBookings.some(other =>
    !sameBookingIdentity(other, booking) &&
    other?.property_id === booking.property_id &&
    String(other?.platform || '').toLowerCase() === 'booking' &&
    isRealReservationCandidate(other, allBookings) &&
    dateOnly(other?.end_date) === dateOnly(booking?.start_date)
  );
}
...
function isBookingFallbackCandidate(booking, allBookings = []) {
  ...
  if (followsConfirmedBookingCheckout(booking, allBookings)) return false;
...
function normalizeBookingsForDisplay(bookings = []) {
  const { byMarker, supersededReservations } = buildCalendarReconciliations(bookings);
  const visible = [];
  for (const booking of bookings) {
    if (supersededReservations.has(booking)) continue;
    const reservation = byMarker.get(booking);
    if (reservation) {
      visible.push(mergeCalendarDatesWithGuest(booking, reservation));
      continue;
    }
