...
Административный интерфейс поддерживает русский и итальянский языки. Публичный календарь уборщицы — на итальянском.
...
|---|---|---|---|
| `/` | Календарь, ближайшие заезды и выезды, синхронизация | администратор | русский / итальянский |
| `/maid` | Уборщицы, объекты и публичные slug-ссылки | администратор | русский / итальянский |
| `/stats` | Сезонная аналитика и история показателей | администратор | русский / итальянский |
| `/tax` | Туристический налог по датам и броням | администратор | русский / итальянский |
| `/reporting` | Comune TXT, Alloggiati, квитанции и ISTAT | администратор | русский / итальянский |
...
5. Брони upsert-ятся в БД. Исчезнувшая запись сначала получает `missing_since` и после grace period становится `active = false`; заменённые актуальным событием технические маркеры Booking.com архивируются сразу. Физического удаления нет.
...
Готовые Scriptable-файлы лежат в `scriptable-widgets/`. Они используют `/api/bookings?widget=today`, `WIDGET_TOKEN` и, если нужно, Vercel Protection bypass token. Виджеты выделяют ближайшие заезды. Реальные токены в репозитории хранить нельзя.

## Email-уведомления мониторинга

Код ежедневной сводки по новым броням и отменам Harmony, Royal и Carina находится в `monitor-app/email-notifier/`. Workflow запускается в `07:17 UTC` и по умолчанию работает в dry-run: не отправляет писем и не меняет историю уведомлений. Реальная отправка требует отдельного включения Actions variable `BOOKING_EMAIL_ENABLED=true` и секрета `MAILGUN_API_KEY`. Подробности — в [инструкции notifier](monitor-app/email-notifier/README.md).
